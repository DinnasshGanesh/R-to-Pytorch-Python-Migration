  ...
  sidebarLayout(
    ...
    mainPanel(
      tabsetPanel(
        ...
        tabPanel("About",
                 h3("About this Tool"),
                 p("This tool helps dimension water storage tanks for irrigated green roofs based on local climate conditions in Germany."),
                 p("It uses data from the German Weather Service (DWD) through the rdwd package on the server."),
                 p("The calculations follow the water balance approach described in the provided Excel spreadsheet by Bjorn Helm."))
      )
    )
  )
)
